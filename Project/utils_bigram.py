...
def show_word_comparison(
    ...
    """
    Shows a comparison of word frequencies and bigram frequencies before and after in post titles for a given community and topic.
    ...
    # Function to generate word and bigram frequencies
    def get_ngrams(text_series):
        word_freq = text_series.apply(str.split).explode().value_counts()
        bigrams = (
            text_series.apply(lambda x: [" ".join(bg) for bg in ngrams(x.split(), 2)])
            .explode()
            .value_counts()
        )
        return word_freq.add(bigrams, fill_value=0)

    # Show top words and bigrams before and after in post title
    word_bigram_counts_after = get_ngrams(posts_after["clean_title"])
    word_bigram_counts_before = get_ngrams(posts_before["clean_title"])

    total_after = word_bigram_counts_after.sum()
    total_before = word_bigram_counts_before.sum()


    word_bigram_comparison = pd.DataFrame(
        {
            "After 3 months": (word_bigram_counts_after / total_after),
            "Before 3 months": (word_bigram_counts_before / total_before),
        }
    ).sort_values(by="After 3 months", ascending=False).head(10)

    plt.figure(figsize=(16, 9))

    sns.barplot(
        data=pd.melt(word_bigram_comparison.reset_index(), id_vars="clean_title"),
        y="clean_title",
        x="value",
        hue="variable",
        palette="viridis",
    ...
    plt.title(
        f"Comparison of Word and Bigram Frequencies Before and After 3 Months\nCommunity {SELECTED_COMMUNITY_IDX}\nTopic: {SELECTED_TOPIC}"
    )

    plt.xlabel("Frequency")
    plt.ylabel("Words and Bigrams in Post Titles")

    plt.legend(title="Time Period")
    plt.tight_layout()

    plt.savefig(
        f"./graphs/word_bigram_comparison_community_{SELECTED_COMMUNITY_IDX}_{SELECTED_TOPIC}.png",
    )

    plt.show()

    return word_bigram_comparison
